...
class KafkaController(val config: KafkaConfig,
  ...
  @volatile private var _brokerEpoch = initialBrokerEpoch // broker的znode的czxid
   ...
   *    初始化一个context上下文对象，缓存当前所有的topic、存活的broker、每个partiton的leader
   * 2. Starts the controller's channel manager 启动controller相关的channel
   * 3. Starts the replica state machine replica状态机
   * 4. Starts the partition state machine 分区状态机
  ...
  private def onControllerFailover(): Unit = {
     ...
     * topic变化: /brokers/topics ！！！
     * 删除topic操作: /admin/delete_topics
     * 日志目录事件：/log_dir_event_notification
     */
    ...
    /**
     * 注册需要监听节点变化的handler：
     * replica重新选举：/admin/preferred_replica_election
     * 重新分配分区：/admin/reassign_partitions
     */
    ...
    /**
     * 状态机启动！！！
     * replicaState状态：主要针对分区的replica（分配到的broker）的分区状态
  ...
  private def initializeControllerContext(): Unit = {
    ...
    // 更新上下文的liveBrokers, 存活broker-Map , key-broker对象，v-epoch
    controllerContext.setLiveBrokers(compatibleBrokerAndEpochs)
    info(s"Initialized broker epochs cache: ${controllerContext.liveBrokerIdAndEpochs}")
    // zk获取所有topic并且注册watcher
    // 然后更新上下文的topic
    controllerContext.setAllTopics(zkClient.getAllTopicsInCluster(true))

    // 1. 给每个topic注册PartitionModificationsHandler，用途是监听分区修改
    registerPartitionModificationsHandlers(controllerContext.allTopics.toSeq)
    // 2. zk获取每个topic的partition分配策略（就是topic节点），并注册上面的wathcer
  ...
  private def processStartup(): Unit = {
    /**
     * 1. 对节点/controller 节点注册controllerChangeHandler，
     * 1) 保存状态监听回调处理到zNodeChangeHandlers
  ...
  private def elect(): Unit = {
    ...
    try {
      /**
       * 2. 尝试竞争controller节点（zk中创建controller，对controller_epoch自增）, 失败会异常
       */
      val (epoch, epochZkVersion) = zkClient.registerControllerAndIncrementControllerEpoch(config.brokerId)
      controllerContext.epoch = epoch // controller_epoch
      controllerContext.epochZkVersion = epochZkVersion // controller_epoch节点znode当前的version
      activeControllerId = config.brokerId // activeControllerId也变成当前节点

      info(s"${config.brokerId} successfully elected as the controller. Epoch incremented to ${controllerContext.epoch} " +
        s"and epoch zk version is now ${controllerContext.epochZkVersion}")

      /**
       * 3. 成为controller后的初始化操作
       */
  ...
  private def processZkPartitionReassignment(): Set[TopicPartition] = {
    ...
    /**
     * 注册watcher并且存不存在/admin/reassign_partitions
     * 存在就执行下面，证明有reassign_partitions
     */
